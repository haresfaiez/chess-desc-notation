...
let movePawn from count : Board.movement = Board.forward Board.Pawn from count
...
let () =
  ...
  describe "PlayTurn" (fun () ->
      test "detects no conflicts in [P-K3, P-Q3]" (fun () ->
        expect (Board.playTurn Board.Pawn (q 3) [(movePawn (k 2) 1)]) |> toBe Board.Moved);
      test "detects a conflict in [P-Q3, Q-Q3]" (fun () ->
        expect (Board.playTurn Board.Queen (q 3) [(movePawn (q 2) 1)]) |> toBe Board.Conflict);
      test "detects a conflict in [P-Q3, P-K3, Q-Q3]" (fun () ->
        expect (Board.playTurn Board.Queen (q 3) [(movePawn (k 2) 1); (movePawn (q 2) 1)]) |> toBe Board.Conflict);
      ...
      test "detects a conflict at Q-Q4 in [P-Q3, P-Q4, Q-Q4]" (fun() ->
        expect (Board.playTurn Board.Queen (q 4) [(Board.Pawn, (q 3), (q 4)); (movePawn (q 2) 1)]) |> toBe Board.Conflict);
    );
  ...
  describe "Forward movement" (fun () ->
      test "moves pawn from P-K2 to P-K3" (fun () ->
          expect(movePawn (k 2) 1) |> toEqual (Board.Pawn, (k 2), (k 3)));
      test "keeps pawn in K2 when the steps count is 0" (fun () ->
          expect(movePawn (k 2) 0) |> toEqual (Board.Pawn, (k 2), (k 2)));
    );
