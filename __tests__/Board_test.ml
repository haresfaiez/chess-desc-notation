...
let () =
  ...
  describe "Movement: P" (fun () ->
      ...
      test "-K3 succeeds from K2" (fun () ->
          expect (Board.move Board.Pawn [(k 2), [(k 3)]] (k 3)) |> toEqual (Board.SMoved (smoveP (k 2) 1)));
      test "-K3 succeeds from initial setup" (fun () ->
          expect (Board.moveOptions Board.Pawn (Board.setup Board.Pawn) (k 3)) |> toEqual [Board.SMoved (smoveP (k 2) 1)]);
    );
  ...
  describe "PlayTurn" (fun () ->
      test "detects no conflicts in [P-K3, P-Q3]" (fun () ->
        expect (Board.turn Board.Pawn (q 3) [(moveP (k 2) 1)]) |> toEqual (Board.SMoved (smoveP (q 2) 1)));
      ...
      test "detects no conflicts in [P-K3]" (fun () ->
        expect (Board.turn Board.Pawn (k 3) []) |> toEqual (Board.SMoved (smoveP (k 2) 1)));
