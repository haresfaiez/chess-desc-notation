...
let () =
  ...
  describe "Movement: P" (fun () ->
      ...
      test "-K3 succeeds from K2" (fun () ->
          expect (Board.move Board.Pawn [(k 2), [(k 3)]] (k 3)) |> toEqual (Board.Moved (smoveP (k 2) 1)));
  ...
  describe "Movement: Q" (fun () ->
      test "-Q4 succeeds from Q1" (fun () ->
        expect (Board.move Board.Queen [source (q 1)] (q 4)) |> toEqual (Board.Moved (smoveQ (q 1) 3)));
    );
  describe "PlayTurn" (fun () ->
      test "detects no conflicts in [P-K3, P-Q3]" (fun () ->
        expect (Board.turn Board.Pawn (q 3) [(moveP (k 2) 1)]) |> toEqual (Board.Moved (smoveP (q 2) 1)));
      ...
      test "detects no conflicts in [P-K3]" (fun () ->
        expect (Board.turn Board.Pawn (k 3) []) |> toEqual (Board.Moved (smoveP (k 2) 1)));
      ...
      test "detects no conflicts in [Q-Q2, Q-Q1, Q-Q2, Q-Q1]" (fun () ->
        ...
        |> toEqual (Board.Moved (smoveQ (q 1) 0)));
